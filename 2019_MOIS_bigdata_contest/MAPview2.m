% function MAP = MAPview2(MAT, PATH)
...
%     sumdata3(:,1:4) = indexdata3;
    ...
    for i = 1:length(AREA)
        ...
        for n = 1:length(position)
            coor1 = rmmissing([SHPemd(position(n)).X; SHPemd(position(n)).Y],2);
            hold on; plot(coor1(1,:), coor1(2,:), 'color', '#a3a3a3');
        end
...
%         set(gca,'xlim',xlimit,'ylim',ylimit);
        ...
        for k = 28-8:28
            ...
            position1 = find(~(temparea-AREAshp(i)));
            for n = 1:length(position1)
                coor1 = rmmissing([SHPemd(position1(n)).X; SHPemd(position1(n)).Y],2);
                hold on; plot(coor1(1,:), coor1(2,:), 'color', '#a3a3a3');
            end
            
            hold on;
            set(gcf,'Visible', 'off', 'position', [0 0 500*20 200]);
            
            rad1 = sumdata2(position,k)/100*8;
            tempfig1 = viscircles(round(coor), rad1, 'color', 'red');
            tempname = '17 '+string(k-5)+'시';
            title(tempname);

            cd(PATH.map);
%             delete(tempfig1);
        end
            saveas(fig, string(AREAname(i)+'2017.png'));
            delete(tempfig1);
        ...
        for k = 28-8:28
            ...
            tempname = '18 '+string(k-5)+'시';
            title(tempname);

            cd(PATH.map);
%             delete(tempfig2);
        end
            saveas(fig, string(AREAname(i)+'2018.png'));
